...
""" for both questions 5 and 6 we check if the fasta file already exists and only do the searches and obtain sequences if false 
    this is we can rerun the program quickly without rebuilding the fasta file each and every time
"""
...
if not(path.isfile(q6_filename)):
    ...
    q6_ids = seq.searchSequences("Lipid+metabolism+AND+organism:3702+AND+fragment:no+AND+length:[100+TO+*]")
