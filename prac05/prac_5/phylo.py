    ...
    def _backwardParsimony(self, aln, seq = None):
        ...
        if self.sequence == None: # no sequence has been assigned
            ...
            if seq == None: # Only root can do this, no parents to consider, so we pick the lowest scoring symbol
                ...
                self.sequence = sequence.Sequence(currbuf, aln.alphabet, self.label, info = 'Inferred by parsimony', gappy = True)
...
if __name__ == '__main__1': 
    ...
    tree.parsimony()
    print tree.root
    print tree.root.getSequence()
    children = tree.root.getDescendants()
    print children[0]
    print children[0].getSequence()
    print children[1]
    print children[1].getSequence()
